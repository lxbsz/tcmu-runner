...
void tcmu_mod_timer(tcmu_timer_t *timer, uint64_t timeout, tcmu_timer_cbk_t cbk)
{
	...
	uv_timer_init(tcmu_uv_loop, timer);
	uv_timer_start(timer, cbk, timeout * 1000, 0);
...
void tcmu_reset_timer(tcmu_timer_t *timer)
{
	 ...
	 */
	uv_timer_start(timer, cbk, timer->expires, timer->repeat);
	pthread_spin_unlock(&timer_lock);
}
#endif
...
//	loop_timer.repeat = 0xefffffffffffffff;
	bzero(&loop_timer, sizeof(uv_timer_t));
	uv_timer_init(tcmu_uv_loop, &loop_timer);
	uv_timer_start(&loop_timer, tcmu_timer_cbk, 5000, 5000);
