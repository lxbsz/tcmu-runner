...
static int xcopy_work_fn(struct tcmu_device *dev, struct tcmulib_cmd *cmd)
{
	...
	max_nolb = min(max_sectors, ((uint32_t)(~0U)));
...
static int handle_rcr_op(struct tcmu_device *dev, struct tcmulib_cmd *cmd)
{
	struct iovec *iovec = cmd->iovec;
	size_t iov_cnt = cmd->iov_cnt;
	uint8_t *sense = cmd->sense_buf;
	 ...
	 */
	val16 = htobe16(RCR_OP_MAX_SG_DESC_COUNT);
