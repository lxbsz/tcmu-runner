...
static int unmap_work_fn(struct tcmu_device *dev, struct tcmulib_cmd *ucmd)
{
	...
	struct tcmulib_cmd *origcmd = desc->origcmd;
	struct unmap_state *state = origcmd->cmdstate;
	tcmur_handler_unmap_fn_t unmap_fn = state->unmap_fn;
	uint64_t offset = desc->offset, length = desc->length;

	ucmd->done = handle_unmap_cbk;

	return unmap_fn(dev, ucmd, offset, length);
}
...
int tcmur_handle_unmap(struct tcmu_device *dev, struct tcmulib_cmd *origcmd,
		       tcmur_handler_unmap_fn_t unmap_fn)
{
	...
	state->refcount = 0;
	state->unmap_fn = unmap_fn;
	origcmd->cmdstate = state;

	ret = handle_unmap(dev, origcmd, bddl, par);
